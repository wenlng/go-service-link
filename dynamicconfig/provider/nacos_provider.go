...
// Reconnect ..
func (p *NacosProvider) Reconnect(ctx context.Context) error {
	p.mu.Lock()
	defer p.mu.Unlock()

	p.pool.Close()
	pool, err := clientpool.NewNacosConfigPool(p.config.poolSize, p.config.address, &p.config.NacosExtraConfig)
	if err != nil {
		return fmt.Errorf("failed to reconnect nacos: %v", err)
	}
	p.pool = pool

	p.outLog(helper.OutputLogTypeInfo, "[NacosProvider] Nacos reconnected successfully")
...
func (p *NacosProvider) HealthCheck(ctx context.Context) HealthStatus {
	...
	respHTTP, err := http.Get("http://" + p.serverAddrs[0] + ":8848/nacos/v1/console/health")
	if err != nil {
